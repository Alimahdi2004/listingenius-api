...
Video generation with official Higgsfield Python SDK
...
# Set Higgsfield credentials for SDK - it uses HF_KEY format
...
async def lifespan(app: FastAPI):
    ...
    if HF_API_KEY:
        print(f"   HF_KEY set: {os.environ.get('HF_KEY', '')[:20]}...")
    yield
...
# ═══════════════════════════════════════════════════════════════
# HIGGSFIELD VIDEO GENERATION - Using Official SDK
# ═══════════════════════════════════════════════════════════════

def generate_video_sync(image_url: str, prompt: str, aspect_ratio: str) -> dict:
    """Synchronous video generation using official SDK"""
    try:
        import higgsfield_client
        
        print(f"      Calling higgsfield_client.subscribe...")
        print(f"      Model: kling/v1.6/pro/image-to-video")
        print(f"      Aspect ratio: {aspect_ratio}")
        
        # Use the SDK's subscribe method
        ...
        if video_url:
            ...
            return {"status": "failed", "error": "No video URL in response", "url": None}
            
    except ImportError as e:
        print(f"      ❌ Import error: {e}")
        return {"status": "failed", "error": f"SDK import failed: {e}", "url": None}
    except Exception as e:
        print(f"      ❌ SDK error: {e}")
        return {"status": "failed", "error": str(e), "url": None}
            ...
            try:
                ...
                for i, (format_name, aspect_ratio) in enumerate(formats):
                    ...
                    result = await loop.run_in_executor(
                        None,
                        lambda ar=aspect_ratio: generate_video_sync(image_url, prompt, ar)
                    )
                    
                    videos[format_name] = result
                    print(f"   {'✅' if result.get('status') == 'completed' else '❌'} {format_name}: {result.get('status')}")
